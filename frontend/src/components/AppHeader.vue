...
<template>
  <header
    class="fixed top-0 left-0 right-0 z-50 flex h-14 items-center gap-4 border-b bg-background px-6"
  >
    <div class="flex items-center gap-2.5">
      <div
        class="w-7 h-7 bg-gradient-to-br from-primary to-primary/80 rounded-lg flex items-center justify-center flex-shrink-0 shadow-sm"
      >
        <Workflow class="w-3.5 h-3.5 text-primary-foreground" :stroke-width="2.5" />
      </div>
      <span class="font-semibold text-foreground tracking-tight"> Flow </span>
    </div>

    <nav class="flex items-center gap-2 text-sm flex-1">
      <template v-for="(crumb, index) in breadcrumbs" :key="index">
        <ChevronRight
          v-if="index === 0 && breadcrumbs.length > 0"
        ...
        <DropdownMenu v-if="crumb.isProjectSelector">
          <DropdownMenuTrigger asChild>
            <button
              class="flex items-center gap-1 text-foreground font-medium hover:bg-muted px-2 py-1 rounded-md transition-colors"
            >
              {{ crumb.label }}
        ...
        <ChevronRight
          v-if="index < breadcrumbs.length - 1 && index > 0"
          class="w-4 h-4 text-muted-foreground"
        />
      </template>
    </nav>
    ...
    <div class="flex items-center gap-3">
      ...
      <DropdownMenu v-if="showQuickActions">
        ...
        <DropdownMenuContent align="end">
          ...
          <DropdownMenuSeparator />
          <DropdownMenuItem @click="handleQuickAction('table')">
            <Plus class="w-4 h-4 mr-2" />
            New Table
          </DropdownMenuItem>
          <DropdownMenuItem @click="handleQuickAction('workflow')">
            <Plus class="w-4 h-4 mr-2" />
            New Workflow
          </DropdownMenuItem>
        </DropdownMenuContent>
